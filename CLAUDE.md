...
- **Instagram feed:** `src/components/InstagramFeed.tsx` renders the 6 latest posts from a Behold.so JSON feed set via `NEXT_PUBLIC_IG_FEED_URL` (see `.env.local.example`); falls back to placeholder tiles until Marni connects @marniblythespeaks on behold.so and the env var is set (locally and in Vercel).
